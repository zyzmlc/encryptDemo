...
/*
 1：生成RSA私钥,base64编码的二进制数据。生成一个1024位的私钥
...
4：从代码中不会使用直接使用pem，会从pem中提取证书。 需要输入一些证书请求信息，参考提示内容。
 openssl req -new -key private.pem -out rsacert.csr
 
 5: 给请求文件 从 pem文件中 提取证书，.crt  这个证书还是base64编码的，这个crt文件也不是直接使用的。 签名一个证书一年5千元。

 openssl x509 -req -days 3650 -in rsacert.csr -signkey private.pem -out rsacert.crt
 
 6:生成der文件.  这个der文件主要包含公钥。最后通过这个来生成一个p12是这个der对应的私钥文件。
 openssl x509 -outform der -in rsacert.crt -out rsacert.der
 
 7：通过crt文件通提取私钥P12文件，公钥也是从crt中提取出来的
